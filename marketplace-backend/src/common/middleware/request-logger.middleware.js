...
var RequestLoggerMiddleware = function () {
    ...
    var RequestLoggerMiddleware = _classThis = /** @class */ (function () {
        ...
        RequestLoggerMiddleware_1.prototype.use = function (req, res, next) {
            var start = Date.now();
            var method = req.method, url = req.url, headers = req.headers, body = req.body;
            res.on('finish', function () {
                var duration = Date.now() - start;
                var statusCode = res.statusCode;
                console.log("[".concat(new Date().toISOString(), "] ").concat(method, " ").concat(url, " ").concat(statusCode, " ").concat(duration, "ms"));
